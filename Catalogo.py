...
if __name__ == '__main__':
    print("--- INICIANDO VALIDACIÓN LOCAL DE MÓDULOS 1 Y 2 (CON IMPORT OS) ---")
    archivo_test = "pacientes_prueba.dat"
    
    pacientes_test = [
        {"dni": 111, "apellido": "Perez", "nombre": "Juan", "telefono": "123456", "prioridad": 1},
        {"dni": 222, "apellido": "Gomez", "nombre": "Maria Ana", "telefono": "456789", "prioridad": 2},
        {"dni": 333, "apellido": "Perez", "nombre": "Carlos", "telefono": "789101", "prioridad": 3},
        {"dni": 444, "apellido": "EsteApellidoEsExtremadamenteLargoYSuperaLosTreintaBytes", "nombre": "Luis", "telefono": "111213", "prioridad": 1}
    ]
    
    # Prueba Módulo 1
    crear_archivo_pacientes(archivo_test, pacientes_test)
    tam_esperado = len(pacientes_test) * TAM_REGISTRO
    tam_real = os.path.getsize(archivo_test)
    
    print(f"[*] Validación de Tamaño con OS: Esperado {tam_esperado} bytes | Real {tam_real} bytes.")
    assert tam_real == tam_esperado, "ERROR: El tamaño del archivo binario no coincide."
    print("-> Módulo 1 superado con éxito.")
    
    # Prueba Módulo 2
    ind_dni, ind_ape = construir_indices(archivo_test)
    print("\n[*] Índices generados en memoria:")
    print("Índice por DNI:", ind_dni)
    print("Índice por Apellido:", ind_ape)
    
    paciente_buscado = buscar_por_dni(archivo_test, ind_dni, 222)
    assert paciente_buscado["apellido"] == "Gomez", "ERROR en búsqueda por DNI."
    print("\n-> Módulo 2 superado con éxito.")
    
    # Limpieza limpia usando OS
    if os.path.exists(archivo_test):
        os.remove(archivo_test)
        print("[*] Archivo temporal de pruebas eliminado correctamente.")